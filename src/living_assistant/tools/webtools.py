...
def build_web_tools(workspace: Workspace, config: dict, approval: ApprovalManager | None = None,
    ...
    def web_fetch(url: str):
        try:
            with httpx.Client(timeout=20.0, headers={'User-Agent':'LivingAssistant/0.9.2'}, trust_env=False) as c:
    ...
    def _stream_download(url: str, dest: Path) -> tuple[int,str,str]:
        ...
        with httpx.Client(timeout=30.0, headers={'User-Agent':'LivingAssistant/0.9.2'}, trust_env=False) as c:
