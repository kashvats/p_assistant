...
from living_assistant.security.security_utils import url_network_scope, safe_display_url, is_sensitive_path, resolve_url_target, redact_secrets
...
class BrowserController:
    ...
    headless: bool = False
    max_sessions: int = 5
    quiet_provider: Callable[[], bool] | None = None
    _sessions: dict = field(default_factory=dict, init=False, repr=False)

    def _quiet_block(self) -> dict | None:
        if self.quiet_provider is None:
            return None
        try:
            if bool(self.quiet_provider()):
                return {'ok': False, 'blocked': True, 'quiet_mode': True, 'error': 'Browser activity is suppressed during focus/quiet hours.'}
        except Exception:
            # A broken preference provider must fail closed for browser activity.
            return {'ok': False, 'blocked': True, 'quiet_mode': True, 'error': 'Browser activity is unavailable while focus/quiet state cannot be verified.'}
        return None
