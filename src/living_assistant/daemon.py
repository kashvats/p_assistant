...
class NervousSystem:
    ...
    def __init__(self, config: dict, memory: MemoryStore, processes: ProcessRegistry | None = None,
        ...
        self.restart_exhausted_notified: set[str] = set()

    def _ports(self):
        audit = audit_local()
        return {str(x.get("local")) for x in audit.get("listening_ports", []) if x.get("local")}

    def _process_events(self) -> list[dict]:
        events = []
        for item in self.processes.list():
            key = item["id"]
            running = bool(item.get("running"))
            prev = self.previous_running.get(key)
            self.previous_running[key] = running
            ...
            if not running and item.get("desired_state") == "running" and item.get("auto_restart"):
                ...
                if result.get("ok"):
                    self.previous_running[key] = True
                    events.append({"kind": "project_process_restarted", "process_id": key, "name": item.get("name"), "restart_count": result.get("restart_count")})
                elif "Maximum automatic restart" in result.get("error", "") and key not in self.restart_exhausted_notified:
                    self.restart_exhausted_notified.add(key)
                    events.append({"kind": "project_restart_exhausted", "process_id": key, "name": item.get("name"), "error": result.get("error")})

            if running and item.get("health_url"):
                try:
                    r = httpx.get(item["health_url"], timeout=2.5, follow_redirects=True)
                ...
                if ok:
                    ...
                    if self.health_failures[key] == 3:
                        events.append({"kind": "project_health_failed", "process_id": key, "name": item.get("name"), "health_url": item.get("health_url")})
        return events

    def _todo_events(self) -> list[dict]:
        events = []
        for todo in self.memory.due_todos():
            events.append({"kind": "todo_due", "todo_id": todo["id"], "title": todo["title"], "due_at": todo["due_at"]})
            self.memory.mark_todo_notified(todo["id"])
        return events
    ...
    def tick(self) -> list[dict]:
        ...
        events.extend(self._process_events())
        ...
        for e in events:
            ...
            if e["kind"] in {"project_process_crashed", "project_process_restarted", "project_restart_exhausted", "project_health_failed", "todo_due", "new_listening_port"}:
                self.notifier.send("Living Assistant", self._event_message(e))
        return events

    @staticmethod
    def _event_message(e: dict) -> str:
        kind = e.get("kind")
