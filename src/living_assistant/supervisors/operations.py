...
OPERATIONS_PROMPTS = {
    "monitoring": "You are a monitoring specialist. Analyze process metrics, CPU/RAM telemetry, and logs.",
    "network": "You are a network specialist. Analyze open ports, active connections, and bandwidth usage.",
    "cloud": "You are a cloud/services specialist. Review container states and background daemon health."
}
...
class OperationsSupervisor:
    ...
    def _execute_specialist(self, role: str, task: str, context: str) -> SpecialistResponse | dict:
        prompt = OPERATIONS_PROMPTS.get(role, OPERATIONS_PROMPTS["monitoring"]) + "\n" + JSON_INSTRUCTION
        messages = [
