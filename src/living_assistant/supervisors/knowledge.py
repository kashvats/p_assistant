...
KNOWLEDGE_PROMPTS = {
    "research": "You are a research specialist. Analyze web search results and extract factual information.",
    "document": "You are a document specialist. Summarize large files, extract key points, and locate citations."
}
...
class KnowledgeSupervisor:
    ...
    def _execute_specialist(self, role: str, task: str, context: str) -> SpecialistResponse | dict:
        prompt = KNOWLEDGE_PROMPTS.get(role, KNOWLEDGE_PROMPTS["research"]) + "\n" + JSON_INSTRUCTION
        messages = [
