...
class Orchestrator:
    def __init__(self, model_manager: ModelManager, model: str, tools: list[Tool],
                 ...
                 max_session_messages: int = 24, experiences=None, event_bus=None, run_history=None, model_usage=None):
    ...
    def _delegate(self, role: str, task: str, context: str = '', _session_id: str | None = None, _run_id: str | None = None):
        # Session/run identifiers are internal execution metadata. They are never part
        # of the model-visible tool schema, but let specialist usage be attributed to
        # the same user run when the concrete router supports that metadata.
        import inspect

        delegate = self.specialists.delegate
        try:
            params = inspect.signature(delegate).parameters.values()
            accepts_metadata = any(p.kind is inspect.Parameter.VAR_KEYWORD for p in params) or {
                'session_id', 'run_id'
            }.issubset(inspect.signature(delegate).parameters)
        except (TypeError, ValueError):
            accepts_metadata = False
        if accepts_metadata:
            return delegate(
                role, task, context, session_id=_session_id, run_id=_run_id
            )
    ...
    def _history_start(self, user_text: str, context: str, session_id: str | None) -> tuple[str | None, str | None]:
        project = self._project_hint(context)
        if not self.run_history:
            return None, project
        try:
            return self.run_history.start_run(user_text, project=project, session_id=session_id), project
        except Exception:
            return None, project

    def _history_tool(self, run_id: str | None, project: str | None, session_id: str | None, name: str | None, args: dict, result):
        if not self.run_history or not run_id or not name:
            return
        try:
            self.run_history.record_tool(run_id, name, args, result, project=project, session_id=session_id)
        except Exception:
            pass

    def _history_finish(self, run_id: str | None, project: str | None, session_id: str | None, status: str, summary: str = ''):
        if not self.run_history or not run_id:
            return
        try:
            self.run_history.finish_run(run_id, project=project, session_id=session_id, status=status, summary=summary)
        except Exception:
            pass

    def _record_model_usage(self, response: dict | None, latency_seconds: float, session_id: str | None, run_id: str | None):
        if not self.model_usage:
            return
        try:
            self.model_usage.record_response(
                self.model, response, latency_seconds, session_id=session_id, run_id=run_id, role='orchestrator'
            )
        except Exception:
    ...
    def _execute_tool(self, name: str | None, args, session_id: str | None = None, run_id: str | None = None):
        ...
        try:
            if name == 'delegate_agent':
                return args, tool.handler(
                    **args, _session_id=session_id, _run_id=run_id
                )
    ...
    def run(self, user_text: str, context: str = '', session_id: str | None = None) -> str:
        run_id, history_project = self._history_start(user_text, context, session_id)
        self._publish('chat.started', session_id=session_id, model=self.model, run_id=run_id)
        if self.resources:
            ...
            if not ok:
                ...
                self._history_finish(run_id, history_project, session_id, 'resource_blocked', reason)
                self._publish('chat.completed', session_id=session_id, model=self.model, run_id=run_id)
        ...
        try:
            for _ in range(self.max_steps):
                ...
                self._record_model_usage(data, usage_elapsed, session_id, run_id)
                ...
                if not tool_calls:
                    ...
                    self._history_finish(run_id, history_project, session_id, 'completed', 'Assistant completed the run.')
                    self._publish('chat.completed', session_id=session_id, model=self.model, run_id=run_id)
                    return self._finish(answer, session_id)
                for call in tool_calls:
                    ...
                    args, result = self._execute_tool(name, args, session_id=session_id, run_id=run_id)
                    self._history_tool(run_id, history_project, session_id, name, args, result)
                    messages.append({'role': 'tool', 'tool_name': name, 'content': json.dumps(result, default=str)[:60000]})
                    self._publish('tool.completed', session_id=session_id, tool=name, ok=bool(result.get('ok', True)) if isinstance(result, dict) else True, run_id=run_id)
                    if self.experiences:
                        try:
            ...
            self._history_finish(run_id, history_project, session_id, 'error', redact_secrets(exc, 1000))
            self._publish('chat.error', session_id=session_id, model=self.model, error=redact_secrets(exc, 500), run_id=run_id)
            raise
        if self.experiences and trace:
            try:
                self.experiences.learn_from_trace(user_text, trace, project=project_hint, session_id=session_id)
        ...
        self._history_finish(run_id, history_project, session_id, 'limited', 'Configured tool-step limit reached.')
        self._publish('chat.completed', session_id=session_id, model=self.model, limited=True, run_id=run_id)
        return self._finish(answer, session_id)
    ...
    def run_stream(self, user_text: str, context: str = '', session_id: str | None = None):
        ...
        run_id, history_project = self._history_start(user_text, context, session_id)
        self._publish('chat.started', session_id=session_id, model=self.model, run_id=run_id)
        yield {'type': 'status', 'status': 'starting', 'model': self.model, 'run_id': run_id}
        if self.resources:
            ok, reason = self.resources.can_start_model()
            if not ok:
                self.mm.sleep()
        ...
        try:
            for step in range(self.max_steps):
                ...
                with self._model_lease() as keep_alive:
                    ...
                    usage_elapsed = time.perf_counter() - usage_started
                self._record_model_usage(last_usage_chunk, usage_elapsed, session_id, run_id)
                ...
                for call in tool_calls:
                    ...
                    self._publish('tool.completed', session_id=session_id, tool=name, ok=ok_result, run_id=run_id)
            ...
            self._history_finish(run_id, history_project, session_id, 'error', redact_secrets(exc, 1000))
            self._publish('chat.error', session_id=session_id, model=self.model, error=redact_secrets(exc, 500), run_id=run_id)
            yield {'type': 'error', 'error': redact_secrets(exc, 1200), 'run_id': run_id}
            return
