...
cp "$WHEEL" "$BASE/dist/"; cp "$ROOT/scripts/install.py" "$BASE/scripts/"; cp "$ROOT/src/living_assistant/release_manager.py" "$ROOT/src/living_assistant/__init__.py" "$BASE/src/living_assistant/"
...
if [ "$#" -eq 0 ]; then WHEEL=$(find "$ROOT/dist" -name 'living_assistant-*.whl' | head -n1); exec python3 "$ROOT/scripts/install.py" install "$WHEEL"; fi
