...
- [ ] **BROKEN-01** · `voice.py` + `cli.py` · Wake word detection triggers but the approval pre-injection in `awake` uses the wrong action string hash. Voice commands are silently dropped after wake word fires.
- [ ] **BROKEN-02** · `experience.py` · Confidence decay (`confidence_half_life_days`) exists in schema and code but `daemon.py` never calls the decay tick. Lessons never actually decay or expire — the memory grows forever.
- [ ] **BROKEN-03** · `task_graph.py` + `tools/planning.py` · The Long-Term DAG planner we built has no connection to the Orchestrator or Runtime. It exists in isolation — nothing calls it.
- [ ] **BROKEN-04** · `orchestrator.py` · `max_steps=10` is hardcoded. Any task requiring more than 10 tool calls silently stops mid-execution with no error message to the user.
- [ ] **BROKEN-05** · `orchestrator.py` · `max_session_messages=12` causes context amnesia on any conversation longer than ~6 exchanges.
- [ ] **BROKEN-06** · `aggregator.py` · `aggregate()` is a stub. Collects results from multiple sources but performs no actual synthesis before returning to the Orchestrator.
- [ ] **BROKEN-07** · `skills.py` · `SkillRegistry` loads from a JSON file but no default skills are shipped. Registry is always empty on a fresh install.
- [ ] **BROKEN-08** · `webui/index.html` · All JavaScript is minified into ~8 lines of hand-concatenated string building. No error handling. Chat has no markdown rendering or code highlighting.
